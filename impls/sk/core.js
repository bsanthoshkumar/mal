...
const { MalSymbol, pr_str, Nil, List } = require("./types");
...
env.set(
  ...
  (...values) => new String(values.map((x) => pr_str(x, true)).join(" "))
);
...
env.set(
  new MalSymbol("str"),
  (...values) => new String(values.map((x) => pr_str(x, false)).join(""))
);
...
env.set(new MalSymbol("count"), (x) => x.length());
